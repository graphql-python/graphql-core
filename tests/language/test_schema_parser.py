...
from graphql.language import (
    BooleanValueNode, DocumentNode, EnumTypeDefinitionNode,
    EnumValueDefinitionNode, FieldDefinitionNode,
    InputObjectTypeDefinitionNode, InputValueDefinitionNode,
    InterfaceTypeDefinitionNode, ListTypeNode, NameNode, NamedTypeNode,
    NonNullTypeNode, ObjectTypeDefinitionNode,
    ObjectTypeExtensionNode, OperationType, OperationTypeDefinitionNode,
    ScalarTypeDefinitionNode, SchemaExtensionNode, DirectiveNode,
    StringValueNode, UnionTypeDefinitionNode, parse)
...
def field_node_with_args(name, type_, args, loc):
    return FieldDefinitionNode(
        name=name, arguments=args, type=type_, directives=[], loc=loc,
        description=None)
...
def enum_value_node(name, loc):
    return EnumValueDefinitionNode(
        name=name_node(name, loc), directives=[], loc=loc,
        description=None)


def input_value_node(name, type_, default_value, loc):
    return InputValueDefinitionNode(
        name=name, type=type_, default_value=default_value, directives=[],
        loc=loc, description=None)
...
def operation_type_definition(operation, type_, loc):
    return OperationTypeDefinitionNode(
        operation=operation, type=type_, loc=loc)
...
def describe_schema_parser():
    ...
    def simple_extension():
        ...
        assert extension.fields == [field_node(
            name_node('world', (23, 28)),
            type_node('String', (30, 36)), (23, 36))]
    ...
    def extension_do_not_include_descriptions():
        assert_syntax_error("""
            ...
            extend type Hello {
              world: String
            }""", "Unexpected Name 'extend'", (3, 13))
        assert_syntax_error("""
          extend "Description" type Hello {
            world: String
          }""", "Unexpected String 'Description'", (2, 18))
    ...
    def schema_extension():
        ...
        assert isinstance(doc, DocumentNode)
        assert doc.loc == (0, 75)
        assert doc.definitions == [schema_extension_node(
            [], [operation_type_definition(OperationType.MUTATION, type_node(
                'Mutation', (53, 61)), (43, 61))], (13, 75))]
    ...
    def simple_non_null_type():
        ...
        assert definition.fields == [field_node(
            name_node('world', (16, 21)),
            non_null_type(type_node('String', (23, 29)), (23, 30)), (16, 30))]
    ...
    def simple_type_inheriting_interface():
        ...
        assert definition.directives == []
        assert definition.fields == [field_node(
            name_node('field', (30, 35)),
            type_node('String', (37, 43)), (30, 43))]
    ...
    def simple_type_inheriting_multiple_interfaces():
        ...
        assert definition.description is None
        assert definition.interfaces == [
            type_node('Wo', (22, 24)), type_node('rld', (27, 30))]
        assert definition.directives == []
        assert definition.fields == [field_node(
            name_node('field', (33, 38)),
            type_node('String', (40, 46)), (33, 46))]
    ...
    def simple_type_inheriting_multiple_interfaces_with_leading_ampersand():
        ...
        assert definition.description is None
        assert definition.interfaces == [
            type_node('Wo', (24, 26)), type_node('rld', (29, 32))]
        assert definition.directives == []
        assert definition.fields == [field_node(
            name_node('field', (35, 40)),
            type_node('String', (42, 48)), (35, 48))]
    ...
    def simple_interface():
        ...
        assert definition.directives == []
        assert definition.fields == [field_node(
            name_node('world', (21, 26)),
            type_node('String', (28, 34)), (21, 34))]
    ...
    def simple_field_with_arg():
        ...
        assert definition.interfaces == []
        assert definition.directives == []
        assert definition.fields == [field_node_with_args(
            name_node('world', (16, 21)),
            type_node('String', (38, 44)), [input_value_node(
                name_node('flag', (22, 26)),
                type_node('Boolean', (28, 35)), None, (22, 35))], (16, 44))]
    ...
    def simple_field_with_arg_with_default_value():
        ...
        assert definition.fields == [field_node_with_args(
            name_node('world', (16, 21)),
            type_node('String', (45, 51)), [input_value_node(
                name_node('flag', (22, 26)),
                type_node('Boolean', (28, 35)),
                boolean_value_node(True, (38, 42)), (22, 42))], (16, 51))]
    ...
    def simple_field_with_list_arg():
        ...
        assert definition.fields == [field_node_with_args(
            name_node('world', (16, 21)),
            type_node('String', (41, 47)), [input_value_node(
        ...
        assert definition.fields == [field_node_with_args(
            ...
            type_node('String', (53, 59)), [
                input_value_node(
                    ...
                    type_node('Boolean', (30, 37)), None, (22, 37)),
                input_value_node(
                    name_node('argTwo', (39, 45)),
                    type_node('Int', (47, 50)), None, (39, 50))], (16, 59))]
    ...
    def union_with_two_types():
        ...
        assert definition.types == [
            type_node('Wo', (14, 16)), type_node('Rld', (19, 22))]
        assert definition.loc == (0, 22)
    ...
    def union_with_two_types_and_leading_pipe():
        ...
        assert definition.directives == []
        assert definition.types == [
            type_node('Wo', (16, 18)), type_node('Rld', (21, 24))]
        assert definition.loc == (0, 24)

    def union_fails_with_no_types():
        assert_syntax_error('union Hello = |',
                            'Expected Name, found <EOF>', (1, 16))

    def union_fails_with_leading_double_pipe():
        assert_syntax_error('union Hello = || Wo | Rld',
                            'Expected Name, found |', (1, 16))

    def union_fails_with_trailing_pipe():
        assert_syntax_error('union Hello = | Wo | Rld |',
                            'Expected Name, found <EOF>', (1, 27))
    ...
    def simple_input_object():
        ...
        assert definition.description is None
        assert definition.directives == []
        assert definition.fields == [input_value_node(
            name_node('world', (17, 22)),
            type_node('String', (24, 30)), None, (17, 30))]
        assert definition.loc == (1, 32)
    ...
    def directive_with_incorrect_locations():
        assert_syntax_error('\ndirective @foo on FIELD | INCORRECT_LOCATION',
                            "Unexpected Name 'INCORRECT_LOCATION'", (2, 27))

    def disallow_legacy_sdl_empty_fields_supports_type_with_empty_fields():
        assert_syntax_error('type Hello { }',
                            'Syntax Error: Expected Name, found }', (1, 14))

    def disallow_legacy_sdl_implements_interfaces():
        assert_syntax_error('type Hello implements Wo rld { field: String }',
                            "Syntax Error: Unexpected Name 'rld'", (1, 26))
