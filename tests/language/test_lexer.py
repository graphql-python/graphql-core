...
from graphql.language import (
    Lexer, Source, SourceLocation, Token, TokenKind)
...
def describe_lexer():
    ...
    def lex_reports_useful_string_errors():
        ...
        assert_syntax_error(
          ...
          'did you mean to use a double quote (")?', (1, 1))
        assert_syntax_error(
          '"contains unescaped \x07 control char"',
          "Invalid character within String: '\\x07'.", (1, 21))
        assert_syntax_error(
            '"null-byte is not \x00 end of file"',
            "Invalid character within String: '\\x00'.", (1, 19))
        assert_syntax_error(
            '"multi\nline"', 'Unterminated string', (1, 7))
        assert_syntax_error(
            '"multi\rline"', 'Unterminated string', (1, 7))
        assert_syntax_error(
            '"bad \\x esc"', "Invalid character escape sequence: '\\x'.",
            (1, 7))
        assert_syntax_error(
            '"bad \\u1 esc"',
            "Invalid character escape sequence: '\\u1 es'.", (1, 7))
        assert_syntax_error(
            '"bad \\u0XX1 esc"',
            "Invalid character escape sequence: '\\u0XX1'.", (1, 7))
        assert_syntax_error(
            '"bad \\uXXXX esc"',
            "Invalid character escape sequence: '\\uXXXX'.", (1, 7))
        assert_syntax_error(
            '"bad \\uFXXX esc"',
            "Invalid character escape sequence: '\\uFXXX'.", (1, 7))
        assert_syntax_error(
            '"bad \\uXXXF esc"',
            "Invalid character escape sequence: '\\uXXXF'.", (1, 7))
    ...
    def lexes_block_strings():
        ...
        assert lex_one('"""contains \\""" triplequote"""') == Token(
            TokenKind.BLOCK_STRING, 0, 31, 1, 1, None,
    ...
    def lex_reports_useful_block_string_errors():
        ...
        assert_syntax_error(
            '"""contains unescaped \x07 control char"""',
            "Invalid character within String: '\\x07'.", (1, 23))
        assert_syntax_error(
            '"""null-byte is not \x00 end of file"""',
            "Invalid character within String: '\\x00'.", (1, 21))
    ...
    def lexes_numbers():
        ...
        assert lex_one('4.123') == Token(
            TokenKind.FLOAT, 0, 5, 1, 1, None, '4.123')
        assert lex_one('-4') == Token(
            TokenKind.INT, 0, 2, 1, 1, None, '-4')
        assert lex_one('-42') == Token(
            TokenKind.INT, 0, 3, 1, 1, None, '-42')
        assert lex_one('-4.123') == Token(
            TokenKind.FLOAT, 0, 6, 1, 1, None, '-4.123')
        assert lex_one('0.123') == Token(
            TokenKind.FLOAT, 0, 5, 1, 1, None, '0.123')
        assert lex_one('123e4') == Token(
            TokenKind.FLOAT, 0, 5, 1, 1, None, '123e4')
        assert lex_one('123E4') == Token(
            TokenKind.FLOAT, 0, 5, 1, 1, None, '123E4')
        assert lex_one('123e-4') == Token(
            TokenKind.FLOAT, 0, 6, 1, 1, None, '123e-4')
        assert lex_one('123e+4') == Token(
            TokenKind.FLOAT, 0, 6, 1, 1, None, '123e+4')
    ...
    def lexes_punctuation():
        ...
        assert lex_one('...') == Token(
            TokenKind.SPREAD, 0, 3, 1, 1, None, None)
        ...
        assert lex_one('[') == Token(
            TokenKind.BRACKET_L, 0, 1, 1, 1, None, None)
        assert lex_one(']') == Token(
            TokenKind.BRACKET_R, 0, 1, 1, 1, None, None)
