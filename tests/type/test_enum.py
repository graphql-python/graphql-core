...
ComplexEnum = GraphQLEnumType('Complex', {
    'ONE': complex1,
    'TWO': complex2})
...
QueryType = GraphQLObjectType('Query', {
    'colorEnum': GraphQLField(ColorType, args={
        ...
        resolve=lambda value, info, **args:
            args.get('fromInt') or
            args.get('fromString') or args.get('fromEnum')),
    'colorInt': GraphQLField(GraphQLInt, args={
        'fromEnum': GraphQLArgument(ColorType),
        'fromInt': GraphQLArgument(GraphQLInt)},
        resolve=lambda value, info, **args:
            args.get('fromInt') or args.get('fromEnum')),
    'complexEnum': GraphQLField(ComplexEnum, args={
        ...
        resolve=lambda value, info, **args:
            ...
            else Complex2() if args.get('provideBadValue')
            else args.get('fromEnum'))})

MutationType = GraphQLObjectType('Mutation', {
    'favoriteEnum': GraphQLField(ColorType, args={
        'color': GraphQLArgument(ColorType)},
            resolve=lambda value, info, color=None: color)})

SubscriptionType = GraphQLObjectType('Subscription', {
    'subscribeToEnum': GraphQLField(ColorType, args={
        'color': GraphQLArgument(ColorType)},
            resolve=lambda value, info, color=None: color)})
...
def describe_type_system_enum_values():
    ...
    def does_not_accept_incorrect_internal_value():
        ...
        assert result == ({'colorEnum': None}, [{
            'message': "Expected a value of type 'Color'"
                       " but received: 'GREEN'",
            'locations': [(1, 3)], 'path': ['colorEnum']}])
    ...
    def accepts_enum_literals_as_input_arguments_to_subscriptions():
        doc = ('subscription ($color: Color!) {'
               ' subscribeToEnum(color: $color) }')
        result = execute_query(doc, {'color': 'GREEN'})
    ...
    def may_be_internally_represented_with_complex_values():
        ...
        assert result == ({
            ...
            [{'message':
                "Expected a value of type 'Complex' but received: Complex2",
                'locations': [(6, 15)], 'path': ['bad']}])
