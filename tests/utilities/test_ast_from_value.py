...
from graphql.language import (
    BooleanValueNode, EnumValueNode, FloatValueNode,
    IntValueNode, ListValueNode, NameNode, NullValueNode, ObjectFieldNode,
    ObjectValueNode, StringValueNode)
from graphql.type import (
    GraphQLBoolean, GraphQLEnumType, GraphQLFloat,
...
def describe_ast_from_value():
    ...
    def converts_boolean_values_to_asts():
        assert ast_from_value(
            True, GraphQLBoolean) == BooleanValueNode(value=True)

        assert ast_from_value(
            False, GraphQLBoolean) == BooleanValueNode(value=False)
        ...
        assert ast_from_value(
            0, GraphQLBoolean) == BooleanValueNode(value=False)

        assert ast_from_value(
            1, GraphQLBoolean) == BooleanValueNode(value=True)
    ...
    def converts_float_values_to_float_asts():
        ...
        assert ast_from_value(
            123.0, GraphQLFloat) == FloatValueNode(value='123')

        assert ast_from_value(
            123.5, GraphQLFloat) == FloatValueNode(value='123.5')

        assert ast_from_value(
            1e4, GraphQLFloat) == FloatValueNode(value='10000')

        assert ast_from_value(
            1e40, GraphQLFloat) == FloatValueNode(value='1e+40')

    def converts_string_values_to_string_asts():
        assert ast_from_value(
            'hello', GraphQLString) == StringValueNode(value='hello')

        assert ast_from_value(
            'VALUE', GraphQLString) == StringValueNode(value='VALUE')

        assert ast_from_value(
            'VA\nLUE', GraphQLString) == StringValueNode(value='VA\nLUE')

        assert ast_from_value(
            123, GraphQLString) == StringValueNode(value='123')

        assert ast_from_value(
            False, GraphQLString) == StringValueNode(value='false')
    ...
    def converts_id_values_to_int_or_string_asts():
        assert ast_from_value(
            'hello', GraphQLID) == StringValueNode(value='hello')

        assert ast_from_value(
            'VALUE', GraphQLID) == StringValueNode(value='VALUE')
    ...
    def converts_list_values_to_list_asts():
        assert ast_from_value(
                ...
                StringValueNode(value='FOO'), StringValueNode(value='BAR')])

        assert ast_from_value(
                ['HELLO', 'GOODBYE'], GraphQLList(my_enum)
            ) == ListValueNode(values=[
                EnumValueNode(value='HELLO'), EnumValueNode(value='GOODBYE')])
    ...
    def converts_input_objects():
        ...
        assert ast_from_value(
            {'foo': 3, 'bar': 'HELLO'}, input_obj) == ObjectValueNode(fields=[
    ...
    def converts_input_objects_with_explicit_nulls():
        ...
        assert ast_from_value({'foo': None}, input_obj) == ObjectValueNode(
            fields=[ObjectFieldNode(
                name=NameNode(value='foo'), value=NullValueNode())])
