...
from graphql.language import (
    parse, print_ast, DirectiveLocation, DocumentNode)
from graphql.pyutils import dedent
from graphql.type import (
    GraphQLArgument, GraphQLDirective, GraphQLEnumType, GraphQLEnumValue,
    GraphQLField, GraphQLID, GraphQLInputField, GraphQLInputObjectType,
    ...
    is_non_null_type, is_scalar_type, specified_directives, validate_schema)
...
def print_test_schema_changes(extended_schema):
    ...
    ast.definitions = [node for node in ast.definitions
...
def describe_extend_schema():
    ...
    def can_be_used_for_limited_execution():
        ...
        result = graphql_sync(extended_schema,
                              '{ newField }', {'newField': 123})
    ...
    def can_describe_the_extended_fields():
        ...
        assert extended_schema.get_type(
            'Query').fields['newField'].description == 'New field description.'
    ...
    def does_not_allow_replacing_a_custom_directive():
        ...
        assert str(exc_info.value).startswith(
            "Directive 'meow' already exists in the schema."
            ' It cannot be redefined.')

    def does_not_allow_replacing_an_existing_type():
        def existing_type_error(type_):
            return (f"Type '{type_}' already exists in the schema."
        ...
        assert str(exc_info.value).startswith(
            existing_type_error('SomeScalar'))
    ...
    def does_not_allow_replacing_an_existing_field():
        def existing_field_error(type_, field):
            return (f"Field '{type_}.{field}' already exists in the schema."
