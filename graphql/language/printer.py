...
class PrintAstVisitor(Visitor):
    ...
    def leave_operation_definition(self, node, *_args):
        ...
        return join([op.value, join([name, var_defs]),
                    directives, selection_set], ' ') if (
                name or directives or var_defs or op != OperationType.QUERY
        ) else selection_set
    ...
    def leave_field(self, node, *_args):
        return join([wrap('', node.alias, ': ') + node.name +
                    wrap('(', join(node.arguments, ', '), ')'),
                    join(node.directives, ' '), node.selection_set], ' ')
    ...
    def leave_schema_definition(self, node, *_args):
        return join(['schema', join(node.directives, ' '),
    ...
    def leave_object_type_definition(self, node, *_args):
        return join(['type', node.name, wrap('implements ',
                    join(node.interfaces, ' & ')),
                    join(node.directives, ' '), block(node.fields)], ' ')
    ...
    def leave_input_object_type_definition(self, node, *_args):
        return join(['input', node.name, join(node.directives, ' '),
                    block(node.fields)], ' ')
    ...
    def leave_schema_extension(self, node, *_args):
        return join(['extend schema', join(node.directives, ' '),
                    block(node.operation_types)], ' ')
    ...
    def leave_object_type_extension(self, node, *_args):
        return join(['extend type', node.name,
                    wrap('implements ', join(node.interfaces, ' & ')),
                    join(node.directives, ' '), block(node.fields)], ' ')

    def leave_interface_type_extension(self, node, *_args):
        return join(['extend interface', node.name, join(node.directives, ' '),
                    block(node.fields)], ' ')

    def leave_union_type_extension(self, node, *_args):
        return join(['extend union', node.name, join(node.directives, ' '),
                    '= ' + join(node.types, ' | ') if node.types else ''], ' ')

    def leave_enum_type_extension(self, node, *_args):
        return join(['extend enum', node.name, join(node.directives, ' '),
                    block(node.values)], ' ')

    def leave_input_object_type_extension(self, node, *_args):
        return join(['extend input', node.name, join(node.directives, ' '),
                    block(node.fields)], ' ')
