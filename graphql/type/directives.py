...
from .definition import (
    GraphQLArgument, GraphQLInputType, GraphQLNonNull, is_input_type)
...
__all__ = [
    ...
    'DirectiveLocation', 'DEFAULT_DEPRECATION_REASON']
...
class GraphQLDirective:
    ...
    def __init__(self, name: str,
        ...
        if not all(isinstance(value, DirectiveLocation)
            ...
            try:
                locations = [
                    value if isinstance(value, DirectiveLocation)
                    else DirectiveLocation[value] for value in locations]
            except (KeyError, TypeError):
                raise TypeError(
                    f'{name} locations must be DirectiveLocation objects.')
        if args is None:
            args = {}
        elif not isinstance(args, dict) or not all(
                isinstance(key, str) for key in args):
            raise TypeError(
                f'{name} args must be a dict with argument names as keys.')
...
def is_specified_directive(directive: GraphQLDirective):
    ...
    return any(specified_directive.name == directive.name
