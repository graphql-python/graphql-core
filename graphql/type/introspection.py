...
from .definition import (
    ...
    GraphQLInputType, GraphQLList, GraphQLNonNull, GraphQLObjectType,
    is_abstract_type, is_enum_type, is_input_object_type,
    is_interface_type, is_list_type, is_named_type, is_non_null_type,
    is_object_type, is_scalar_type, is_union_type)
from ..pyutils import is_invalid
from .scalars import GraphQLBoolean, GraphQLString
from ..language import DirectiveLocation

__all__ = [
    'SchemaMetaFieldDef', 'TypeKind',
    'TypeMetaFieldDef', 'TypeNameMetaFieldDef',
    'introspection_types', 'is_introspection_type']
...
__Directive: GraphQLObjectType = GraphQLObjectType(
    ...
    fields=lambda: {
        ...
        'name': GraphQLField(
            ...
            resolve=lambda obj, _info: obj.name),
...
__Type: GraphQLObjectType = GraphQLObjectType(
    ...
    fields=lambda: {
        'kind': GraphQLField(
            ...
            resolve=TypeFieldResolvers.kind),
        'name': GraphQLField(
            GraphQLString, resolve=TypeFieldResolvers.name),
        'description': GraphQLField(
            GraphQLString, resolve=TypeFieldResolvers.description),
        'fields': GraphQLField(
            GraphQLList(GraphQLNonNull(__Field)),
            args={'includeDeprecated': GraphQLArgument(
        ...
        'interfaces': GraphQLField(
            ...
            resolve=TypeFieldResolvers.interfaces),
        ...
        'ofType': GraphQLField(
            __Type, resolve=TypeFieldResolvers.of_type)})
        ...
        if is_object_type(type_) or is_interface_type(type_):
            ...
            if not includeDeprecated:
                return [item for item in items
                        if not item[1].deprecation_reason]
            return list(items)
...
__Field: GraphQLObjectType = GraphQLObjectType(
    ...
    fields=lambda: {
        'name': GraphQLField(
            GraphQLNonNull(GraphQLString),
            resolve=lambda item, _info: item[0]),
        'description': GraphQLField(
            GraphQLString,
            resolve=lambda item, _info: item[1].description),
        'args': GraphQLField(
            GraphQLNonNull(GraphQLList(GraphQLNonNull(__InputValue))),
            resolve=lambda item, _info: (item[1].args or {}).items()),
        'type': GraphQLField(
            GraphQLNonNull(__Type),
            resolve=lambda item, _info: item[1].type),
        'isDeprecated': GraphQLField(
            GraphQLNonNull(GraphQLBoolean),
            resolve=lambda item, _info: item[1].is_deprecated),
        'deprecationReason': GraphQLField(
            GraphQLString,
            resolve=lambda item, _info: item[1].deprecation_reason)})
...
__InputValue: GraphQLObjectType = GraphQLObjectType(
    ...
    fields=lambda: {
        ...
        'defaultValue': GraphQLField(
            ...
            resolve=lambda item, _info:
                None if is_invalid(item[1].default_value) else print_value(
                    item[1].default_value, item[1].type))})
