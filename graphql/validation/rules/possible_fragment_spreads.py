...
def type_incompatible_spread_message(
        frag_name: str, parent_type: str, frag_type: str) -> str:
    return (f"Fragment '{frag_name}' cannot be spread here as objects"
            f" of type '{parent_type}' can never be of type '{frag_type}'.")


def type_incompatible_anon_spread_message(
        parent_type: str, frag_type: str) -> str:
    return (f'Fragment cannot be spread here as objects'
            f" of type '{parent_type}' can never be of type '{frag_type}'.")
...
class PossibleFragmentSpreadsRule(ValidationRule):
    ...
    def enter_inline_fragment(self, node, *_args):
        ...
        if (is_composite_type(frag_type) and is_composite_type(parent_type) and
                not do_types_overlap(context.schema, frag_type, parent_type)):
    ...
    def enter_fragment_spread(self, node, *_args):
        ...
        parent_type = context.get_parent_type()
        if frag_type and parent_type and not do_types_overlap(
                context.schema, frag_type, parent_type):
            context.report_error(GraphQLError(
                type_incompatible_spread_message(
