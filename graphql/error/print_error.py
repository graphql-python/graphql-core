...
def print_error(error: 'GraphQLError') -> str:
    ...
    if error.nodes:
        for node in error.nodes:
            if node.loc:
                print_location(highlight_source_at_location(
                    node.loc.source,
...
def highlight_source_at_location(
        source: 'Source', location: 'SourceLocation') -> str:
...
def print_prefixed_lines(lines: List[Tuple[str, Optional[str]]]) -> str:
    ...
    pad_len = reduce(
        lambda pad, line: max(pad, len(line[0])), existing_lines, 0)
    return '\n'.join(map(
        lambda line: line[0].rjust(pad_len) + line[1],  # type:ignore
        existing_lines))
